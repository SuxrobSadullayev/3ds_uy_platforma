...
import { BadgeCheck, Box, Camera, MapPin, Maximize, BedDouble, Building, Layers } from 'lucide-react'
import {
  type Property,
...
export function PropertyCard({ property, onToggleCompare, isCompared }: PropertyCardProps) {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card transition-shadow hover:shadow-lg">
      <Link href={`/mulklar/${property.id}`} className="flex flex-col flex-1">
      ...
      {/* Solishtirish (Compare) Action Button */}
      {onToggleCompare && (
        <button
          type="button"
          onClick={(e) => {
          ...
          className={`absolute top-3 right-3 flex size-8 items-center justify-center rounded-lg border shadow-sm backdrop-blur-md transition-all ${
            isCompared
              ? 'border-primary bg-primary text-primary-foreground'
              : 'border-border/60 bg-background/80 text-muted-foreground hover:bg-muted hover:text-foreground'
          }`}
          title={isCompared ? "Solishtirishdan chiqarish" : "Solishtirishga qo'shish"}
          aria-label={`${property.title} ni solishtirishga qo'shish`}
        >
