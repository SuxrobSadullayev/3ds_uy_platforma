...
## 🚀 MAJBURIY GIT WORKFLOW QOIDASI

Har bir vazifa / prompt bajarilib, kod yozib bo'lingach:
1. **Git Commit:** O'zbek tilida aniq, tushunarli va mazmunli commit xabari yozilsin (masalan: `feat: 3D GLTF viewer va solishtirish modali yaratildi`).
2. **Git Push:** O'zgarishlar darhol `git@github.com:SuxrobSadullayev/3ds_uy_platforma.git` masofaviy omboriga push qilinsin (`git push origin main` yoki faol tarmoqqa).
